...
export function ShopDisplay() {
    ...
    // APPLY STEALTH FILTER: Remote users with reservationTime > now + settings.remoteBufferMinutes are hidden on shop display
    const isVisibleShop = (c: Client) => {
        if (c.status !== 'waiting') return false;

        // BYPASS: If they have arrived (status waiting but local/manual or arrived from remote)
        if (c.source === 'manual' || c.status === 'waiting') {
            // If they are remote but have arrived (logic for arrival not fully shown but we check if they are within time)
            // For now, let's assume they are visible if they are in the 'waiting' pool and don't meet stealth criteria
        }

        if (c.source === 'remote' && c.reservationTime) {
            const timeUntil = c.reservationTime - currentTime.getTime();
            const bufferMs = (settings?.remoteBufferMinutes || 30) * 60 * 1000;
            return timeUntil <= bufferMs; // Dynamic Stealth Filter
        }
        return true;
    };

    const shopVisibleClients = clients.filter(isVisibleShop);
    const nextAvailableQueue = shopVisibleClients.filter(c => c.barberPreference === 'next_available');

    return (
        <div className="min-h-screen bg-background text-text-main font-sans selection:bg-primary/30 overflow-hidden grid grid-cols-[16px_1fr_16px]">
            {/* Left Buffer */}
            <div className="h-full bg-background border-r border-border/10"></div>

            {/* Content Shell */}
            <div className="flex flex-col h-full overflow-hidden">
                {/* --- Header --- */}
                <header className="h-20 border-b border-border flex items-center justify-between bg-surface/50 backdrop-blur-md sticky top-0 z-50 px-4">
                ...
                {/* --- Main Grid Content --- */}
                <main className="flex-1 p-4 grid grid-cols-4 gap-6 content-start overflow-hidden">
                    ...
                    <section className="col-span-1 flex flex-col gap-4 h-full">
                        ...
                        <div className="flex-1 bg-surface/30 rounded-2xl border border-border/50 p-4 overflow-y-auto space-y-3 shadow-inner no-scrollbar">
                    ...
                    {barbers.map(barber => {
                        ...
                        // Specific Waiting for this barber (with Stealth Filter applied)
                        const barberQueue = shopVisibleClients.filter(c => c.barberPreference === barber.id);
                        ...
                        return (
                            <section key={barber.id} className="col-span-1 flex flex-col gap-4">
                                    ...
                                    {clientInChair ? (
                                        <div className="absolute inset-0 bg-surface border border-primary/30 rounded-2xl shadow-[0_0_15px_rgba(197,160,89,0.15)] p-5 flex flex-col justify-between overflow-hidden group">
                                            ...
                                            <div className="absolute top-4 right-4 z-10">
                                                <ProgressRing progress={75} size={72} strokeWidth={6} />
                                            </div>
                                        </div>
                                    ) : (
                                ...
                                <div className="flex-1 bg-surface/10 rounded-2xl border border-border/30 p-3 min-h-[200px] overflow-hidden flex flex-col">
            ...
            {/* Right Buffer */}
            <div className="h-full bg-background border-l border-border/10"></div>
        </div>
    );
}
